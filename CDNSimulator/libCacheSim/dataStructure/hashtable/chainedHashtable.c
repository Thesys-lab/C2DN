...
static inline cache_obj_t *chained_hashtable_find_obj_id(hashtable_t *hashtable,
  ...
  hashtable->n_lookup += 1;
  hashtable->n_chain_walk += depth;

  if (hashtable->n_chain_walk / hashtable->n_lookup > 3) {
    WARNING("average hash chain walk %.4lf, hash power %u, curr items %lu\n",
            (double) hashtable->n_chain_walk / hashtable->n_lookup, hashtable->hash_power,
            (unsigned long) (hashtable->n_cur_item));
    hashtable->n_chain_walk = 0;
    hashtable->n_lookup = 0;
  }
  return NULL;
}
...
void _chained_hashtable_expand(hashtable_t *hashtable) {
  DEBUG("chained hash table expand to hash power %d\n", hashtable->hash_power+1);
...
void chained_hashtable_add_ptr_to_monitoring(hashtable_t *hashtable,
  ...
  if (hashtable->n_monitored_ptrs == hashtable->n_allocated_ptrs) {
    ...
    memcpy(hashtable->monitored_ptrs,
           old_ptrs,
