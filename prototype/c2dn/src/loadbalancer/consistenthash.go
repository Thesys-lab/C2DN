...
// modify from https://github.com/serialx/hashring,
// as of Aug.20, 2018, the version in GitHub is not working due to MD5 misuse
...
func (h *HashRing) GetNodes(stringKey string, size int) (nodes []string, ok bool) {
	pos, ok := h.GetNodePos(stringKey)
	if !ok {
		slogger.Fatal("failed to get node pos")
		return nil, false
	}

	if size > len(h.nodes) {
		slogger.Fatal("asking more %v nodes, however only have %v nodes", size, len(h.nodes))
		return nil, false
	}
